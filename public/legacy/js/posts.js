...
function render(posts=[]){
  ...
  postsFeed.innerHTML=posts.length?posts.map(post=>{const media=image(post),date=post.publicado_en?new Intl.DateTimeFormat('es-PE',{day:'2-digit',month:'short',year:'numeric'}).format(new Date(post.publicado_en)).toUpperCase():'';return `<article class="studio-post"><header><div class="post-avatar">SB</div><div><b>SKYBLOCK STUDIO</b><span>${esc(date)}</span></div><i>•••</i></header><div class="post-copy"><h2>${esc(post.titulo)}</h2><p>${esc(post.descripcion)}</p></div>${media?`<img src="${esc(media.url_segura)}" alt="${esc(media.texto_alternativo||post.titulo)}">`:''}</article>`}).join(''):'<div class="posts-empty"><h2>Aún no hay publicaciones.</h2><p>Las historias aparecerán aquí cuando se publiquen desde el panel.</p></div>';
}
render([]);
