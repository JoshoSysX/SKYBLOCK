...
export default function App() {
  ...
  const cargarAdmin = async () => {
    ...
    const ps = (productos.data ?? []).map((p: any) => { const images=[...(p.imagenes || [])].sort((a:FilaImagen,b:FilaImagen)=>(a.posicion||0)-(b.posicion||0)); const sizes=Object.fromEntries((p.tallas || []).map((t:any)=>[t.talla,t.stock])); return { id:p.id, name:p.nombre, type:p.tipo?.nombre || '', collection:p.coleccion?.nombre || '', price:Number(p.precio), description:p.descripcion, materials:p.materiales || '', sizes, limited:p.es_limitado, limitedUnits:p.unidades_limitadas, stockBySize:Boolean(p.stock_por_talla), stockUnlimited:Boolean(p.stock_ilimitado), stockAvailable:p.stock_disponible ?? Object.values(sizes).reduce((sum:number,stock:any)=>sum+Number(stock||0),0), blocked:p.estado === 'archivado', image:images[0]?.url_segura || '', gallery:images.slice(1).map((i:FilaImagen)=>i.url_segura) } })
      ...
      if (String(e.data?.tipo || '').startsWith('SKYBLOCK_ADMIN_') && !['SKYBLOCK_ADMIN_GUARDAR_POST','SKYBLOCK_ADMIN_ELIMINAR_POST','SKYBLOCK_ADMIN_ELIMINAR_MENSAJE'].includes(e.data.tipo)) {
        ...
        try {
          ...
          if (e.data.tipo === 'SKYBLOCK_ADMIN_GUARDAR_COLECCION') {
            ...
            const payload={producto_id:producto.id,coleccion_id:coleccion.id,codigo_hmac:`\\x${d.hash}`,codigo_admin:String(d.code||'').trim().toUpperCase()||null,ultimos_cuatro:String(d.codeHint||'').slice(-4),numero_serie:d.series,estado:d.status==='blocked'?'bloqueado':'disponible',propietario_nombre:d.owner==='Sin registrar'?null:d.owner,creado_por:user.id}
  ...
  return <iframe ref={frame} className="legacy-frontend" src={urlLegacy(rutaInicial, location.search)} title={BRAND} onLoad={enviar} />
}
